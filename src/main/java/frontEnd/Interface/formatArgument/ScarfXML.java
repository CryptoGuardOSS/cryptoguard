...
public class ScarfXML implements TypeSpecificArg {
     ...
     */
    public Boolean inputValidation(EnvironmentInformation info, List<String> args) throws ExceptionHandler {
        ...
        if (cmd.hasOption(argsIdentifier.SCONFIG.getId())) {
            ...
            Properties SWAMPProperties = loadProperties(cmd.getOptionValue(argsIdentifier.SCONFIG.getId()));
            ...
            info.setAssessmentFramework(cmd.getOptionValue(StringUtils.trimToNull(argsIdentifier.SASSESSFW.getId()), null));
            info.setAssessmentFrameworkVersion(cmd.getOptionValue(StringUtils.trimToNull(argsIdentifier.SASSESSFWVERSION.getId()), null));
            info.setAssessmentStartTime(cmd.getOptionValue(StringUtils.trimToNull(argsIdentifier.SASSESSMENTSTARTTS.getId()), null));
            info.setBuildFramework(cmd.getOptionValue(StringUtils.trimToNull(argsIdentifier.SBUILDFW.getId()), null));
            info.setBuildFrameworkVersion(cmd.getOptionValue(StringUtils.trimToNull(argsIdentifier.SBUILDFWVERSION.getId()), null));
            info.setBuildRootDir(cmd.getOptionValue(StringUtils.trimToNull(argsIdentifier.SBUILDROOTDIR.getId()), null));
            info.setPackageName(cmd.getOptionValue(StringUtils.trimToNull(argsIdentifier.SPACKAGENAME.getId()), null));
            info.setPackageRootDir(cmd.getOptionValue(StringUtils.trimToNull(argsIdentifier.SPACKAGEROOTDIR.getId()), null));
            info.setPackageVersion(cmd.getOptionValue(StringUtils.trimToNull(argsIdentifier.SPACKAGEVERSION.getId()), null));
            info.setParserName(cmd.getOptionValue(StringUtils.trimToNull(argsIdentifier.SPARSERFW.getId()), null));
            info.setParserVersion(cmd.getOptionValue(StringUtils.trimToNull(argsIdentifier.SPARSERFWVERSION.getId()), null));
    ...
    public Options getOptions() {
        ...
        for (argsIdentifier arg : Listing.ScarfXML.retrieveArgs()) {
            Option tempConfig = Option.builder(arg.getId()).hasArg().argName(arg.getArgName()).desc(arg.getDesc()).build();

            if (arg.hasDefaultArg())
                tempConfig.setType(String.class);
